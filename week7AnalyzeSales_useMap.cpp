...
// Cấu trúc để lưu thông tin đơn hàng
struct Order {
...
int main() {
    vector<Order> orders; // Danh sách lưu trữ các đơn hàng
    unordered_map<string, int> shopRevenue; // Bảng băm lưu tổng doanh thu của mỗi cửa hàng
    ...
    string line; // Chuỗi lưu từng dòng dữ liệu đầu vào
    ...
    while (getline(cin, line) && line != "#") { // Đọc từng dòng truy vấn cho đến khi gặp ký tự '#'
        ...
        if (query == "?total_number_orders") {
            ...
            for (const Order &order : orders) { // Duyệt qua từng đơn hàng
                int orderTime = timeToSeconds(order.timePoint); // Chuyển thời gian của đơn hàng sang giây
                if (orderTime >= start && orderTime <= end) { // Kiểm tra nếu thời gian đơn hàng nằm trong khoảng thời gian yêu cầu
                    periodRevenue += order.price; // Cộng giá của đơn hàng vào doanh thu của khoảng thời gian
                }
            }
